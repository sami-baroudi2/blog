...
$allArticlesRequest = $db->query('SELECT * FROM `articles` WHERE `id_categorie` =3 ORDER BY `id`');
...
<html lang="fr">
  <head>
    ...
    <title>Les articles de catégorie 3</title>
  </head>
  <body>
    ...
    <main>
      ...
      <div class="titre-index-zone">
          <p1 class="titre-index">Les articles de catégorie 3</p1><br />
        </div>
        <article>
          <div class="table-wrapper">
            <table class="fl-table">
              ...
              <tbody>
                <?php
                $articles = $db->query('SELECT * FROM `articles` WHERE `id_categorie` =3 ORDER BY `id` DESC LIMIT '.$start.','.$articles_par_page);
