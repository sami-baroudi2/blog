...
$allArticlesRequest = $db->query('SELECT * FROM `articles` WHERE `id_categorie` =2 ORDER BY `id`');
...
<html lang="fr">
  <head>
    ...
    <title>Les articles de catégorie 2</title>
  </head>
  <body>
    ...
    <main class="footer-auto-bottom">
      ...
      <div class="titre-index-zone">
          <p1 class="titre-index">Les articles de catégorie 2</p1><br />
        </div>
        <article>
          <div class="table-wrapper">
            <table class="fl-table">
              ...
              <tbody>
                <?php
                $articles = $db->query('SELECT * FROM `articles` WHERE `id_categorie` =2 ORDER BY `id` DESC LIMIT '.$start.','.$articles_par_page);
